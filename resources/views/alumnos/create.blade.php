...
@section('title', __('messages.create_alumno'))

@section('content')
<div class="max-w-2xl mx-auto">
    <h2 class="text-3xl font-bold mb-6">{{ __('messages.create_alumno') }}</h2>

    <form action="{{ route('alumnos.store') }}" method="POST" class="bg-white p-6 rounded shadow">
        @csrf
        ...
        <div class="mb-6">
            ...
            <textarea id="direccion" name="direccion" rows="4"
                      class="w-full px-3 py-2 border border-gray-300 rounded">{{ old('direccion') }}</textarea>
        </div>

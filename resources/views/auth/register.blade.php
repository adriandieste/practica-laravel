...
<div class="max-w-md w-full mx-auto bg-white rounded-lg shadow-md p-8">
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <!-- Name -->
        <div class="mb-4">
            <label for="name" class="block text-gray-700 font-bold mb-2">{{ __('messages.name') }}</label>
            <input type="text" id="name" name="name" value="{{ old('name') }}" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-600" required>
            @error('name')
